...
export async function getStaticPaths() {
  const res = await fetch(`https://cms.techwire.biz/wp-json/wp/v2/categories`);
...
export async function getStaticProps({ params }) {
  const res = await fetch(`https://cms.techwire.biz/wp-json/wp/v2/categories?slug=${params.slug}`);
  const category = await res.json();

  const postsRes = await fetch(`https://cms.techwire.biz/wp-json/wp/v2/posts?categories=${category[0].id}`);
